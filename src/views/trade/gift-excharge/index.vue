              ...
              <el-col :span="24">
                <el-form-item label="批量账号" prop="batchAccounts">
                  <el-input
                    ...
                    type="textarea"
                    :rows="5"
                    placeholder="请输入账号，每行格式：账号 密码"
                  />
                  <div class="form-tip">
                    每行一条记录，格式：账号 密码（中间可以是多个空格）
                  </div>
                </el-form-item>
              </el-col>
            </el-row>
...
const loadTemplates = async () => {
  try {
    ...
    // 确保返回的模板数据中数值字段都是Number类型
    templateList.value = (res.data.list || []).map(item => ({
      ...item,
      totalAmount: Number(item.totalAmount),
      days: Number(item.days),
      multipleBase: Number(item.multipleBase),
      floatAmount: Number(item.floatAmount),
...
const loadTemplate = (template: any) => {
  ...
  form.floatAmount = Number(template.floatAmount);
  form.intervalHours = Number(template.intervalHours);
