...
export function usePostHook() {
  ...
  const timeRange = computed<[string, string] | null>({
    get() {
      if (searchFormParams.begin_time && searchFormParams.end_time) {
        return [searchFormParams.begin_time, searchFormParams.end_time];
      } else {
        return null;
      }
    },
    set(v) {
      if (v?.length === 2) {
        searchFormParams.begin_time = v[0];
        searchFormParams.end_time = v[1];
      } else {
        searchFormParams.begin_time = undefined;
        searchFormParams.end_time = undefined;
      }
    }
  });
